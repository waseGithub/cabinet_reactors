...
# Modified version of minimalmodbus for RS485 receivers
# Engineer: Serdar Coskun
# Compnay: WASE Limited
# Description: Transmitter is controlled by a GPIO pin to enable transmitter and receiver
#	       Please see enTx to re-configure Tx driver pin number on Pi
...
#Transmitter enable pin on Pi4
enTx = 18

GPIO.setwarnings(False)
GPIO.setmode(GPIO.BCM)
GPIO.setup(enTx, GPIO.OUT)
GPIO.output(enTx, GPIO.LOW)
...
class Instrument:
	...
	#Modified for rs485 transceivers

	#Enable transmitter to write
	GPIO.output(enTx, GPIO.HIGH)
	time.sleep(.00001)

        self.serial.write(request)

	#Disable transmitter after writing, switches to receiver mode
	#use 0.0085 for 9600 baud rate and 0.0042 for 19200 baud rate
	#For different baud rates please scope transmitted signal and optimise the delay
	time.sleep(.0042)
	GPIO.output(enTx, GPIO.LOW)
